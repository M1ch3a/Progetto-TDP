...
async function loadNews() {
  ...
  try {
    ...
    const API_KEY = '29301ef09e5c426a96143b785681a53b'; // Chiave per NewsAPI
    
    // 4. Fetch dei dati (cerca articoli su mafia/camorra/ndrangheta in italiano)
    const response = await fetch(
      `https://newsapi.org/v2/everything?q=mafia OR camorra OR 'ndrangheta&language=it&sortBy=publishedAt&apiKey=${API_KEY}`
    );

    // 5. Verifica se la risposta è OK (status 200-299)
    if (!response.ok) throw new Error('Network response was not ok');
    
    // 6. Converti la risposta in JSON
    const data = await response.json();

    // 7. Mostra gli articoli (massimo 6) o fallback
    displayNews(
      container,
    ...
    // 8. Gestione errori: log in console e fallback
